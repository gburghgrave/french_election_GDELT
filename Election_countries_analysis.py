...
events = coll.aggregate([{'$match': {'ActionGeo_CountryCode': {'$in': [
                        'FR', 'BE', 'IT', 'DE', 'PL', 'PT', 'NL', 'SP', 'SE']}}}], allowDiskUse=True)
data = list(events)
events.close()
...
df.groupby([pd.Grouper(key='SQLDATE', freq='D'), 'ActionGeo_CountryCode'])[
...
# ACTOR 1
events = coll.aggregate([{'$match': {'Actor1Geo_CountryCode': {'$in': [
                        'FR', 'BE', 'IT', 'DE', 'PL', 'PT', 'NL', 'SP', 'SE']}}}], allowDiskUse=True)
data = list(events)
events.close()
df = pd.DataFrame(data)
